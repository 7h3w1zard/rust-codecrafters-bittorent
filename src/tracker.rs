...
pub struct TrackerResponse {
    /// An integer, indicating how often(secs) your client should make a request to the tracker.
...
mod peers {
    ...
    impl<'de> Visitor<'de> for PeersVisitor {
        ...
        fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
            formatter.write_str(
                "6 bytes: 4 bytes are the peer's IP address, 2 bytes are the peer's port number.",
            )
        }
